...
if [ -z "${LIBCLANG_PATH:-}" ]; then
    ...
    # works on an arm64 build host as well as x86_64. The filename glob has to
    # cover the versioned SONAMEs Debian/Ubuntu ship -- libclang-21.so and
    # libclang-21.so.1 exist there with no unversioned libclang.so alongside
    # them, so a plain libclang.so* glob reports "not installed" on a host that
    # has libclang-dev. /usr/lib/llvm-*/lib is where the unversioned symlink
    # lives on those distros; clang-sys searches all of these itself.
    for dir in /usr/lib64 /usr/lib /usr/lib/*-linux-gnu /usr/local/lib \
               /usr/lib/llvm-*/lib /usr/lib64/llvm-*/lib; do
...
if [ "$use_gcc" = 1 ]; then
    ...
    #
    # The wrapper also drops `-Wl,-O<n>`. rustc emits `-Wl,-O1` for every
    # optimized build when it thinks the linker is GNU-flavoured; zig >= 0.16
    # answers with "ignoring deprecated linker optimization setting '1'", which
    # rustc's linker_messages lint then reports as a warning. lld ignores the
    # setting either way, so removing it changes nothing about the output --
    # and it keeps genuine linker diagnostics visible, which blanket-allowing
    # the lint would not.
    mkdir -p "$TOOLS_DIR/bin"
    cat > "$TOOLS_DIR/bin/zig" <<EOF
#!/bin/sh
# Generated by musl.sh -- do not edit.
argc=\$#
while [ "\$argc" -gt 0 ]; do
    arg="\$1"
    shift
